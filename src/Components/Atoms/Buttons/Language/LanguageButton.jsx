    ...
    return (
        <div className="flex flex-row gap-3">
            <button disabled={disabledEs} onClick={handleClick} className="relative bg-transparent w-10 p-0 hover:border-transparent">
                <ES  className={`absolute top-0 left-0 z-1 w-10 h-10  hover:cursor-pointer hover:scale-110 ${disabledEn? "filter grayscale":null}`}/>
            </button>
            <button disabled={disabledEn} onClick={handleClick} className="bg-transparent p-0 w-fit h-fit hover:border-transparent">
                <GB  className={`w-10 h-10 hover:cursor-pointer hover:scale-110 ${disabledEs? "filter grayscale":null}`}/>
            </button>
        </div>
    );
}
